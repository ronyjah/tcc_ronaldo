...
class Test274b:
    ...
    def __init__(self,config,app):
        ...
        self.msg = self.__config.get('tests','2.7.4b')
        self.msg_lan =self.__config.get('tests','2.7.4b')
    ...
    def run_Lan(self):
        ...
        while not self.__queue_lan.full():
            ...
            if not self.__config_setup_lan.get_setup_OK():
                ...
                logging.info('Setup LAN concluido. Verificando o recebimento do DNS Src list Opt ')
                self.set_status_lan('Setup LAN concluido. Verificando o recebimento do DNS Src list Opt')
                if self.__config_setup_lan.get_recvd_dhcp_srcladdr():

                    logging.info('Aprovado Teste 2.7.4b: DNS List Option OK.')
                    self.set_status_lan('APROVADO Teste 2.7.4b: DNS List Option OK.')
                    time.sleep(2)
                    ...
                    logging.info(' LAN: Teste2.7.4b: Reprovado. Não foi recebido DNS List Option.')
                    self.set_status_lan('LAN: Teste2.7.4b: Reprovado. Não foi recebido DNS List Option')
                    time.sleep(2)
                    self.set_status_lan('REPROVADO') # Mensagem padrão para o frontEnd atualizar Status
                    self.__packet_sniffer_lan.stop()
                    self.__finish_wan = True 
    ...
    def run(self):
        ...
        self.__packet_sniffer_wan = PacketSniffer('Test274b-WAN',self.__queue_wan,self,self.__config,self.__wan_device_tr1)
        self.__packet_sniffer_wan.start()
        
        self.__packet_sniffer_lan = PacketSniffer('Test274b-LAN',self.__queue_lan,self,self.__config,self.__lan_device)
        ...
        while not self.__queue_wan.full():
            ...
            wrpcap("WAN-2.7.4b.cap",cache_wan)
            ...
            if not self.__config_setup1_1.get_setup1_1_OK():
                ...
                if not self.__finish_wan: 
                    logging.info('WAN: Respondendo ao DHCP renew')
                    self.set_status('WAN: Respondendo ao DHCP renew')
